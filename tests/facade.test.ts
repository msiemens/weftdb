...
import { createWeftDb, inProcessTransport, WeftClient, WeftDb } from "weftdb/client";
...
test("a row written through the facade is the row the client holds", async () => {
  await fc.assert(
    fc.asyncProperty(
      fc.string({ maxLength: 12 }),
      fc.boolean(),
      fc.option(fc.integer({ min: -1000, max: 1000 }), { nil: undefined }),
      async (title, done, weight) => {
        const { db: database, client } = db();
        const todos = database.collection("todos");
        await todos.create("todo-1", { title, done, ...(weight === undefined ? {} : { weight }) });
...
test("a field left out is left alone, not written as empty", async () => {
  const { db: database, client } = db();
  const todos = database.collection("todos");
  await todos.create("todo-1", { title: "plan", done: false });
  ...
  await todos.update("todo-1", { notes: undefined });
...
test("the facade's writes are ordinary writes: they sync, merge and converge", async () => {
  ...
  await first.collection("todos").create("todo-1", { title: "plan", done: false });
  await firstClient.syncWith(inProcessTransport(server), schemaHash(schema));
  await second.syncWith(inProcessTransport(server), schemaHash(schema));

  assert.equal(secondDb.collection("todos").get("todo-1")?.fields.get(fieldName("title")), "plan");

  await secondDb.collection("todos").update("todo-1", { done: true });
  await second.syncWith(inProcessTransport(server), schemaHash(schema));
  await firstClient.syncWith(inProcessTransport(server), schemaHash(schema));
  assert.equal(first.collection("todos").get("todo-1")?.fields.get(fieldName("done")), true);
});

test("listing a collection returns that collection and nothing else", async () => {
  const { db: database } = db();
  await database.collection("todos").create("todo-1", { title: "plan", done: false });
  await database.collection("todos").create("todo-2", { title: "other", done: false });
  await database.collection("todo_events").create("event-1", { kind: "added" });
...
test("deleting through the facade removes the row and leaves a tombstone to push", async () => {
  const { db: database, client } = db();
  const todos = database.collection("todos");
  await todos.create("todo-1", { title: "plan", done: false }, txnId("create"));
  await todos.delete("todo-1", txnId("remove"));
...
test("a transaction id passed in is the one the ops carry", async () => {
  ...
  const { db: database, client } = db();
  const todos = database.collection("todos");
  await todos.create("todo-1", { title: "plan", done: false }, txnId("mine"));
  await todos.update("todo-1", { title: "changed" }, txnId("mine-too"));

  const transactions = new Set(client.outbox.map((op) => String(op.txnId)));
  assert.deepEqual([...transactions].sort(), ["mine", "mine-too"]);
});

test("an event log has no update or delete on its facade", async () => {
  ...
  await events.create("event-1", { kind: "added" });

  await assert.rejects(() => events.delete("event-1"), /append-class/u);
...
test("event-log rows created through the facade sync as append-class rows", async () => {
  const server = new WeftServer(() => 1_000);
  const { db: database, client } = db();

  await database.collection("todo_events").create("event-1", { kind: "added" }, txnId("event"));
...
test("the facade is a view of one client, not a copy of its state", async () => {
  const { db: database, client } = db();
  ...
  await client.create(tableName("todos"), rowId("todo-1"), { [fieldName("title")]: "direct" }, txnId("direct"));
  assert.equal(database.collection("todos").get("todo-1")?.fields.get(fieldName("title")), "direct");

  await database.collection("todos").update("todo-1", { title: "through the facade" });
  assert.equal(
...
test("a declared json type is the type the facade takes, without a cast", async () => {
  ...
  await views.create("view-1", { label: "by title", sort });

  assert.deepEqual(client.getRow(tableName("views"), rowId("view-1"))?.fields.get(fieldName("sort")), sort);
});

test("a nullable field takes null, and takes it as the type the schema declares", async () => {
  const { db: database, client } = db();
  const todos = database.collection("todos");
  ...
  await todos.create("todo-1", { title: "plan", done: false, weight });
  ...
  await todos.update("todo-1", { weight: 3 });
  await todos.update("todo-1", { weight: null, notes: null });
  const row = client.getRow(tableName("todos"), rowId("todo-1"));
  assert.equal(row?.fields.get(fieldName("weight")), null);
  assert.equal(row?.fields.get(fieldName("notes")), null);
});

test("a json field with nothing declared still takes what the wire can carry", async () => {
  ...
  await createWeftDb(client, open)
    .collection("notes")
