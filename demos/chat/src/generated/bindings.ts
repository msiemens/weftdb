...
export function messagesMutators(client: WeftClient, notify: () => void = () => undefined): MessagesMutators {
  return {
    create(id: string, values: MessagesMutation): void {
      client.append(messagesTable, rowId(id), wire(values), txnId(`create-${id}`));
      notify();
    },
  };
}
...
export function devicesMutators(client: WeftClient, notify: () => void = () => undefined): DevicesMutators {
  return {
    create(id: string, values: DevicesMutation): void {
      client.create(devicesTable, rowId(id), wire(values), txnId(`create-${id}`));
      notify();
    },
    update(id: string, values: DevicesMutation): void {
      client.update(devicesTable, rowId(id), wire(values), txnId(`update-${id}-${crypto.randomUUID()}`));
      notify();
    },
    delete(id: string): void {
      client.delete(devicesTable, rowId(id), txnId(`delete-${id}-${crypto.randomUUID()}`));
      notify();
    },
  };
}
