 ...
 * is written once and never changes, and a device record is rewritten every few seconds.
 */
export const schema = defineSchema({
  // Append-class rows are created once, never edited and never deleted, so two devices posting
  // at the same moment simply add two separate rows. The generated mutators carry `create`
  // alone, and the server rejects anything else against these rows.
  messages: S.eventLog({
    body: S.string(),
