...
| Durability       | Whether this device's database outlives the window. `ephemeral` where a browser declines OPFS, such as private browsing.                     | [Multi-tab coordination](/concepts/multi-tab/)            |
...
| Follower         | A tab that does not hold the database, and reaches the worker holding it over a `MessagePort`.                                               | [Multi-tab coordination](/concepts/multi-tab/)            |
...
| Leader           | The tab holding the Web Lock for a scope, which is the tab that created the worker owning the database.                                      | [Multi-tab coordination](/concepts/multi-tab/)            |
...
| Port broker      | The `SharedWorker` that carries a `MessagePort` from a tab that wants the database to the tab holding it.                                    | [Multi-tab coordination](/concepts/multi-tab/)            |
