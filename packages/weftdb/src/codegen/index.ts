...
export function generateBindings(schema: SchemaDefinition): string {
  ...
  const header = [
    ...
    'import type { MaterializedRow, ReactiveSqlQuery, ScopedRowQuery, TypedQueryKey } from "weftdb/client";',
    // Its own entry point, because the mutators are typed against the shape a write goes through
    // rather than against `WeftClient`: a `WeftClientMirror` standing in for a client that lives in
    // a worker satisfies it too, which is what lets an application on OPFS use generated writes.
    'import type { MutationTarget } from "weftdb/client/mutation-target";',
